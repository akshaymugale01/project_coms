...
const Dashboard = () => {
  ...
  // *** NEW: fetch notices from given API when component mounts ***
  useEffect(() => {
    const fetchNotices = async () => {
      try {
        const res = await fetch("https://app.myciti.life/notices/communicaions_dashboard.json?token=4a2e8ce5b36a3ecf86505017fb13435ad64c9d13c759b606");
        const data = await res.json();
        // Assuming the response has a property like `notices` or an array at root
        // Adjust this line depending on actual JSON structure
    ...
    >
      ...
      <div className=" w-full flex lg:mx-3 flex-col bg-gray-900 overflow-hidden">
        ...
        {notices && notices.length > 0 && (
          <div className="m-5">
            <h2 className="text-white text-xl font-semibold mb-3">Notices</h2>
            <div className="bg-gray-800 p-5 rounded-lg shadow-custom-all-sides space-y-4 max-h-80 overflow-y-auto">
              {notices.map((notice, idx) => (
                <div key={idx} className="p-3 bg-gray-700 rounded">
                  {/* Adjust these fields based on notice JSON structure */}
                  <h3 className="text-white font-bold">{notice.title || notice.subject || "No Title"}</h3>
                  <p className="text-gray-200">{notice.message || notice.body || JSON.stringify(notice)}</p>
                  <p className="text-gray-400 text-sm">{notice.date || notice.created_at || ""}</p>
                </div>
              ))}
            </div>
          </div>
