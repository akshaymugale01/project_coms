  ...
  return (
    <div className="modal">
      <div className="modal-content">
        <h2>{entry ? "Edit Journal Entry" : "Create Journal Entry"}</h2>
        ...
        {formData.entries.map((row, index) => (
          <div key={index} className="entry-row">
            <select
              value={row.ledger_id}
              onChange={(e) =>
                handleChange(index, "ledger_id", e.target.value)
              }
            >
            ...
            <button onClick={() => removeEntryRow(index)}>Remove</button>
          </div>
        ))}
